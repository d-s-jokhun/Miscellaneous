...
%input tau in first column of 'raw'. Do not include 0.
%input MSD/tau in subsequent columns of 'raw'. Do not include 0.
%% Written by D.S.Jokhun on 24.04.2017
...
linear_range=60:99  ;  %the range of x (in terms of row numbers) for which gradient of the straight line is required. E.g. Gradint for tau=15s to tau=25s for telomeres

result_alpha_at_specific_point=[];
result_alpha_series=[];



for curve_num=1:(size(raw,2))-1
    clearvars -except 'raw' 'curve_num' 'smoothening_pts' 'linear_range' 'result*'
    var(:,1)=raw(:,1);
    var(:,2)=raw(:,curve_num+1);
    ...
    result_alpha_at_specific_point(curve_num,1)=alpha_at_specific_point;
    ...
    result_alpha_series(:,curve_num)=alpha_series(:,2);
...
result_alpha_series=horzcat(alpha_series(:,1),result_alpha_series);